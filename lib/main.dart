...
// z (0..1) → ระยะเป็น "เมตรจริง" ด้วยสมการกำลังสอง
double zToMetersQuad(double z) {
  ...
  if (!m.isFinite) return 0.0;

  // ใช้เฉพาะช่วง 1.5–3.5 m ถ้านอกช่วงให้ return 0
  if (m < 1.5 || m > 3.5) return 0.0;

  return m;
}

// ฟังก์ชัน bucket เดิม จะไม่ถูกเรียกใช้แล้ว แต่จะเก็บไว้เฉย ๆ ก็ได้
...
// --------- ยืมชื่อคลาสให้ blob ----------
const double kBorrowMinDetScore = 0.30; // ลดเกณฑ์ความมั่นใจ YOLO ให้ยืมชื่อได้ง่ายขึ้น
const double kBorrowMinIoU = 0.10; // ลด IoU ขั้นต่ำ ทำให้กรอบที่ทับกันน้อยก็ยังนับ
const double kBorrowMinIoM = 0.25; // ลด IoM ขั้นต่ำเช่นกัน
const double kBorrowMaxZDiff = 0.30; // อนุญาตให้ระยะ z ต่างกันได้มากขึ้น
const double kBorrowMaxCenterDist = 0.8; // อนุญาตให้ center ห่างกันมากขึ้นแต่ยังถือว่าใกล้
...
String? _bestLabelForRect(
  ...
  // จุดศูนย์กลางของ blob (Obstacle)
  final double cxBlob = blobRect.center.dx;
  final double cyBlob = blobRect.center.dy;
  final double diagBlob = math.sqrt(
    blobRect.width * blobRect.width +
    blobRect.height * blobRect.height,
  );
  ...
  for (final d in dets) {
    ...
    // คำนวณระยะห่างของจุดศูนย์กลางกรอบ det vs blob
    final double cxDet = d.rect.center.dx;
    final double cyDet = d.rect.center.dy;
    final double dx = cxDet - cxBlob;
    final double dy = cyDet - cyBlob;
    final double centerDist = math.sqrt(dx * dx + dy * dy);

    final double diagDet = math.sqrt(
      d.rect.width * d.rect.width +
      d.rect.height * d.rect.height,
    );
    final double baseDiag = math.max(diagBlob, diagDet);
    final double normCenterDist =
        baseDiag > 0 ? centerDist / baseDiag : 999.0;

    final bool goodOverlap = (iou >= minIoU || iom >= minIoM);
    final bool closeCenter = normCenterDist <= kBorrowMaxCenterDist;

    // ถ้าไม่มี overlap เลย และจุดศูนย์กลางก็ห่างกันมาก → ตัดทิ้ง
    if (!goodOverlap && !closeCenter) continue;

    if (zBlob != null && d.zRel != null) {
      if ((zBlob - d.zRel!).abs() > maxZDiff) continue;
    }

    // น้ำหนักรวม: คิดจาก score * ov * น้ำหนัก class
    final ov = math.max(iou, iom);
    final w = d.score * (ov + 0.5 * (closeCenter ? 1.0 : 0.0)) * _riskW(d.label);
    if (w > bestScore) {
      bestScore = w;
      bestLabel = d.label;
    }
...
class ObstacleDecider {
  ...
  _AlertDecision decide({
    ...
    // 1b) Candidate from BLOB (ใช้ z จาก blob ตรง ๆ)
    for (final b in blobs) {
      final double areaFrac =
          (b.bboxInPreview.width * b.bboxInPreview.height) /
              (frameArea + 1e-6);
      final double cy =
          (b.bboxInPreview.top + b.bboxInPreview.bottom) * 0.5;
    ...
    // 2) กรองความใกล้ด้วย z (ใช้ threshold เดิม)
    ...
    for (final n in near) {
      ...
      final bool veryRisky = (n.label == 'person');
    ...
    // 4) แยกเป้าหมายเป็นสองกลุ่ม: obstacle (BLOB) ก่อน, YOLO+mono ทีหลัง
    final blobConfirmed =
        confirmed.where((c) => c.fromBlob).toList(growable: false);
    final detConfirmed =
        confirmed.where((c) => !c.fromBlob).toList(growable: false);

    ({Rect rect, String? label, double z, bool fromBlob})? target;
    bool obstacleMode = false;

    // 4a) ถ้ามี obstacle (blob) ให้สิทธิ์ก่อนเสมอ → เลือกอันที่ใกล้สุดตาม z
    if (blobConfirmed.isNotEmpty) {
      blobConfirmed.sort((a, b) => b.z.compareTo(a.z)); // z มาก = ใกล้
      target = blobConfirmed.first;
      obstacleMode = true;
    } else if (detConfirmed.isNotEmpty) {
      // 4b) ถ้าไม่มี obstacle เหลือ → ให้ YOLO+mono ทำงาน ในช่วง 1.5–3.5 m
      double bestDetM = 9999.0;
      for (final c in detConfirmed) {
        final double m = zToMetersQuad(c.z);
        if (m < 1.5 || m > 3.5) continue;
        if (m < bestDetM) {
          bestDetM = m;
          target = c;
        }
      }
      // ถ้าไม่มี DET ที่เข้าเงื่อนไขระยะเลย ก็ไม่แจ้งเตือน
      if (target == null) {
        _rollFrames(null);
        return _AlertDecision(shouldAlert: false);
      }
    } else {
      // กันเคสแปลก ๆ ที่ไม่เหลืออะไร
      _rollFrames(null);
      return _AlertDecision(shouldAlert: false);
    }

    // 4c) ตรวจ persistence จากหลายเฟรม (กัน blob กระพริบกรอบ)
    final bool persisted = _isPersisted(target.rect);
    ...
    // 5) สร้างข้อความเสียง
    final String dir =
        _dirFromRect(target.rect, previewSize.width, mirrorX: mirrorX);
    String msg;
    // พยายามใช้ชื่อคลาสจาก target.label ทั้งในกรณี BLOB และ DET
    if (obstacleMode) {
      // === กรณี BLOB: ถือว่าใกล้มาก (<1.5 m) แต่ถ้ามี label จาก YOLO ให้พูดชื่อนั้นแทนคำว่า "สิ่งกีดขวาง" ===
      final String label = (target.label ?? 'สิ่งกีดขวาง');
      msg = "มี ${label} ด้านหน้า ${dir}";
    } else {
      // === กรณี DET: ใช้ระยะเมตรจริงจากสมการกำลังสอง และใช้เฉพาะช่วง 1.5–3.5 m ===
      final String label = (target.label ?? 'สิ่งกีดขวาง');
      final double m = zToMetersQuad(target.z);

      // ป้องกันหลุดเงื่อนไขระยะอีกรอบ (กันเคส edge)
      if (m < 1.5 || m > 3.5) {
        return _AlertDecision(shouldAlert: false);
      }

      final String distStr = m.toStringAsFixed(2); // เช่น 2.34
      msg = "มี ${label} ระยะ ${distStr} เมตรด้านหน้า ${dir}";
    }
  ...
  bool _isPersisted(Rect candidate) {
    if (_lastBoxes.isEmpty) return true;
    int hits = 0;
    for (final r in _lastBoxes) {
      if (r == Rect.zero) continue;
      final double ov = _iouRect(r, candidate);
      if (ov >= 0.2) hits++;
    }
