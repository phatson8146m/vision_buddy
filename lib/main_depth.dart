...
class _CameraPageState extends State<CameraPage> {
  ...
  // ====== FPS rounds logger (10 รอบ x 60 วินาที) ======
  static const int _kRounds = 10;
  static const int _kSecsPerRound = 60;
  final List<List<double>> _fpsRounds =
      List<List<double>>.generate(_kRounds, (_) => <double>[]);
  int _roundIdx = 0;
  int _secsInRound = 0;
  Timer? _roundTimer;
  ...
  Future<void> _initAll() async {
    try {
      ...
      // เริ่มเก็บค่า FPS เป็นรอบ 10 × 60 วินาที
      _startFpsLogging();
    } catch (e) {
  ...
  // ====== FPS logging (10 รอบ x 60 วินาที) ======
  void _startFpsLogging() {
    if (_loggingActive) return;
    // รีเซ็ตค่าทั้งหมด
    for (final r in _fpsRounds) r.clear();
    _roundIdx = 0;
    _secsInRound = 0;
    _loggingActive = true;
    ...
    _roundTimer = Timer.periodic(const Duration(seconds: 1), (t) {
      ...
      // บันทึกค่า FPS ปัจจุบันของทุกวินาที
      _fpsRounds[_roundIdx].add(_fpsNow);
      _secsInRound++;

      // log ลง console เผื่ออยากเซฟจาก Logcat
      debugPrint("Round ${_roundIdx + 1}, Sec $_secsInRound, FPS=${_fpsNow.toStringAsFixed(2)}");

      // ครบ 60 วิ → ขึ้นรอบใหม่
      if (_secsInRound >= _kSecsPerRound) {
        _roundIdx++;
        _secsInRound = 0;

        if (_roundIdx >= _kRounds) {
          // ครบ 10 รอบแล้ว
          t.cancel();
          _loggingActive = false;
          _showFpsSummaryDialog();
        }
      }

      // อัปเดต UI เล็กน้อย (แถบสถานะรอบ)
      if (mounted) setState(() {});
    });
  }
  ...
  double _min(List<double> xs) =>
      xs.isEmpty ? 0.0 : xs.reduce((a, b) => a < b ? a : b);

  double _max(List<double> xs) =>
      xs.isEmpty ? 0.0 : xs.reduce((a, b) => a > b ? a : b);

  void _showFpsSummaryDialog() {
    // คำนวณผลสรุปต่อรอบ + รวมทั้งหมด
    final perRound = <String>[];
    final all = <double>[];
    for (int i = 0; i < _kRounds; i++) {
      final r = _fpsRounds[i];
      if (r.isNotEmpty) {
        perRound.add(
          "รอบ ${i + 1}: avg=${_avg(r).toStringAsFixed(2)}, "
          "min=${_min(r).toStringAsFixed(2)}, "
          "max=${_max(r).toStringAsFixed(2)}  (n=${r.length})"
        );
        all.addAll(r);
      } else {
        perRound.add("รอบ ${i + 1}: — (ไม่มีข้อมูล)");
      }
    }
    final overall = all.isEmpty
        ? "—"
        : "รวมทั้งหมด: avg=${_avg(all).toStringAsFixed(2)}, "
          "min=${_min(all).toStringAsFixed(2)}, "
          "max=${_max(all).toStringAsFixed(2)}  (n=${all.length})";

    // ทำ CSV เผื่อก๊อปไปใช้
    final buffer = StringBuffer();
    buffer.writeln("round,second,fps");
    for (int i = 0; i < _kRounds; i++) {
      final r = _fpsRounds[i];
      for (int s = 0; s < r.length; s++) {
        buffer.writeln("${i + 1},${s + 1},${r[s].toStringAsFixed(4)}");
      }
    }
    ...
    showDialog(
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          title: const Text("สรุปผล FPS (ครบ 10 นาที)", style: TextStyle(color: Colors.white)),
          content: SizedBox(
      ...
      body: camReady && previewSize != null
          ? Stack(
              ...
              children: [
                ...
                // สถานะรอบเก็บข้อมูล (แสดงใต้ปุ่ม Heatmap)
                SafeArea(
                  child: Align(
                    alignment: Alignment.topRight,
                    child: Container(
                      ...
                      child: Text(
                        _loggingActive
                          ? "เก็บรอบ: ${_roundIdx + 1}/$_kRounds • วินาที: ${_secsInRound}/$_kSecsPerRound"
                          : (_roundIdx >= _kRounds
                              ? "ครบ 10 รอบแล้ว"
                              : "พร้อมเริ่มเก็บใหม่ (_startFpsLogging)"),
                        style: const TextStyle(color: Colors.white, fontSize: 12),
                      ),
                    ),
                  ),
